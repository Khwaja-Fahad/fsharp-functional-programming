...
function Main(){
  const num1Var=_c.Create_1("");
  const num2Var=_c.Create_1("");
  const resultVar=_c.Create_1("Click an operation below");
  const onCalculate=(op) => {
    let o=0;
    const _2=Number(num1Var.Get());
    let _3=isNaN(_2)?false:(o=_2,true);
    const _4=[_3, o];
    let o_1=0;
    const _5=Number(num2Var.Get());
    let _6=isNaN(_5)?false:(o_1=_5,true);
    const _7=[_6, o_1];
    if(_4[0]&&_7[0]){
      const b=_7[1];
      const a=_4[1];
      const m=calculate(a, b, op);
      if(m==null)resultVar.Set("Cannot divide by zero!");
      else resultVar.Set(String(a)+" "+String(op)+" "+String(b)+" = "+String(m.$0));
    }
    else resultVar.Set("Please enter valid numbers!");
  };
  const _1=Doc.Element("div", [], [Doc.Element("h1", [], [Doc.TextNode("F# Calculator")]), Doc.Element("p", [], [Doc.TextNode("First Number:")]), Doc.Input([], num1Var), Doc.Element("p", [], [Doc.TextNode("Second Number:")]), Doc.Input([], num2Var), Doc.Element("p", [], [Doc.Element("button", [Attr.HandlerImpl("click", () =>() => onCalculate("+"))], [Doc.TextNode(" + ")]), Doc.Element("button", [Attr.HandlerImpl("click", () =>() => onCalculate("-"))], [Doc.TextNode(" - ")]), Doc.Element("button", [Attr.HandlerImpl("click", () =>() => onCalculate("*"))], [Doc.TextNode(" * ")]), Doc.Element("button", [Attr.HandlerImpl("click", () =>() => onCalculate("/"))], [Doc.TextNode(" / ")])]), Doc.Element("h2", [], [Doc.TextView(resultVar.View)])]);
  LoadLocalTemplates("");
  Doc.RunById("main", _1);
}
function calculate(a, b, op){
  return op=="+"?Some(add(a, b)):op=="-"?Some(subtract(a, b)):op=="*"?Some(multiply(a, b)):op=="/"?divide(a, b):null;
}
function add(a, b){
  return a+b;
}
