...
function Main(){
  const inputVar=_c.Create_1("");
  const resultVar=_c.Create_1("Enter a number above \ud83d\udc46");
  const _1=Doc.Element("div", [], [Doc.Element("h1", [], [Doc.TextNode("Even / Odd Checker")]), Doc.Element("p", [], [Doc.TextNode("Enter a number:")]), Doc.Input([], inputVar), Doc.Element("button", [Attr.HandlerImpl("click", () =>() => {
    let o;
    const m=(o=0,[TryParse(inputVar.Get(), {get:() => o, set:(v) => {
      o=v;
    }}), o]);
    if(m[0]){
      const number=m[1];
      resultVar.Set("Number "+String(number)+" is: "+String(checkEvenOdd(number)));
    }
    else resultVar.Set("\u26a0\ufe0f Please enter a valid number!");
  })], [Doc.TextNode("Check")]), Doc.Element("p", [], [Doc.TextView(resultVar.View)])]);
  LoadLocalTemplates("");
  Doc.RunById("main", _1);
}
